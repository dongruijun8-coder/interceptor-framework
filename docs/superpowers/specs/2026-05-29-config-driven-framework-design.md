...
### 4.4 output_mapping 字段映射规则

`output_mapping` 左边是框架统一字段名，右边是映射来源。优先级：

| 右边值特征 | 行为 | 示例 |
|-----------|------|------|
| 包含 `{{...}}` | 模板变量替换 | `"category": "{{_iter.name}}"` |
| 包含 `.` 且不以 `{{` 开头 | JSON path 取值 | `"name": "room_name"` → `item["room_name"]` |
| 不以 `{{` 开头，不含 `.` | 硬编码字面量 | `"type": "voice"` → 所有行 type 固定为 `"voice"` |
...
class EncryptionProcessor(BaseProcessor):
    ...
    def derive_key(self, client) -> None:
        """当 config 中 key=null 时调用。从 client 上下文派生实际 key。
        例如: aes-cbc 读取 client.config[\"device_token\"] 做 sha256 派生。"""
        ...
...
AccountManager 保留不动。Auth 处理器封装它，不替代它：

```
Auth Processor (password-login)
  ├── 读取 config.json → 知道 endpoint / fields / response_mapping
  ├── 读取 runtime.json → 拿到 phone / password
  ├── 调用 AccountManager.encrypt_phone()  ← 复用现有加密
  ├── 构造请求 → HTTP POST → 解析响应
  └── 返回 token/uid → 写入 runtime.json
```

`account_manager.py` 继续负责：手机号 AES 加密、易盾滑块验证、多账号 CRUD。Auth 处理器只负责「用声明式配置驱动认证流程」，具体加解密委托给 AccountManager。
...
| 层级 | 错误 | 行为 |
|------|------|------|
| encryption.encode | JSON 序列化失败 / AES 加密失败 | 跳过当前用户 → 记录 failed → 下一人 |
| signing.sign | p1/p2/p3 生成失败 | 同上（不应发生，属于配置错误） |
| HTTP 请求 | 超时/连接拒绝/HTTP 4xx/5xx | 房间排行失败 → 跳过整间房。私信失败 → 跳过当前用户 |
| encryption.decode | 解密失败 / JSON 解析失败 | 房间排行失败 → 跳过整间房 |
| 全部房间遍历失败 | >= 3 间连续失败 | 暂停 Pipeline，通知用户检查 |

以上行为与现有 BaseClient 的 try/except 保持一致。处理器自身不处理错误，向上抛出异常，由 BaseClient.run_room 统一 catch。
